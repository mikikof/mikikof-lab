...
echo "  2. Read skills/visual-column/SKILL.md and components.md before coding"
echo "  3. Decide on chapter count (3 / 5 / 6-with-prelude / 7)"
echo "  4. Choose a color palette from templates/palettes.md"
echo "  5. Fill in all {{PLACEHOLDER}} markers in $TARGET_FILE"
echo "  6. Duplicate the chapter stub for each chapter, and the TOC/rail/drawer entries to match"
echo "  7. Add visual elements (.dgm / .timeline / .tier etc) per components.md"
echo "  8. Add references (≥2)"
echo "  9. Update index.html with the new article card"
echo " 10. Run: bash scripts/check.sh $TARGET_FILE"
echo " 11. Run: bash scripts/publish.sh \"$SLUG\" \"Add article: $TITLE\""
