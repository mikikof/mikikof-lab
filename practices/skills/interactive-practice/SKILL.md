...
2. 本ファイル(`SKILL.md`) ― 技術仕様・モバイル UI 機構・落とし穴(「どう」側)
3. `components.md` ― 部品カタログ(コピペ元)
4. `examples/01-01-information-and-media.html` ― **最新の動いている完成品リファレンス**(モバイル UI v2 完備、新規開発のコピー元)
   - 旧版の `examples/01-03-intellectual-property.html` も同等機構を持つが、まずは 01-01 を参照する
...
- **サマリスコアのカウントアップ**:`animateCounter` で 0 → 実値へ easeOutCubic
- **水平スクロール領域とスワイプの分離**:`isInsideHorizontalScroll` ガード(§9.4b)
- **採点後スクロールは sticky topbar 直下**:`block: 'center'` 禁止、手動計算(§9.4c)
- **`touch-action: manipulation`**:iOS 二重タップズーム遅延を解消(§9.4d)
...
CSS(v2 — backdrop + 大きい scale + ゴールドリング + spotPulse オーバーシュート):
```css
.spotlight-backdrop {
  position: fixed; inset: 0;
  background: radial-gradient(ellipse at center, rgba(10,25,47,0.55), rgba(8,18,36,0.78));
...
body.has-spotlight-active .spotlight-backdrop {
  opacity: 1; visibility: visible; pointer-events: auto;
  transition: opacity 0.34s var(--ease), visibility 0s linear 0s;
}
...
.has-spotlight > .cc-cat-item:not(.spotlight) {
  opacity: 0.18; filter: saturate(0.4) blur(1px); transform: scale(0.97);
}
.ip-rich.spotlight, .bd-card.spotlight, .lk-card.spotlight, .cc-cat-item.spotlight {
  transform: scale(1.18) translateY(-4px);
  z-index: 90; position: relative;
  box-shadow:
    0 0 0 3px rgba(212,168,82,0.55),
    0 32px 70px -18px rgba(0,0,0,0.45),
    0 14px 32px -6px rgba(74,120,200,0.32);
  animation: spotPulse 0.42s var(--ease);
}
@keyframes spotPulse {
  0%   { transform: scale(1) translateY(0); }
  60%  { transform: scale(1.22) translateY(-6px); box-shadow: 0 0 0 6px rgba(212,168,82,0.45); }
...
body.has-spotlight-active .spotlight-hint { opacity: 1; }
```

JS でバックドロップ要素 + hint ラベルを `body` に1回だけ append、`document.body.classList.toggle('has-spotlight-active')` で制御する。フォーカス時は `requestAnimationFrame` 経由で対象カードを `window.innerHeight / 2` を中心に手動スクロール。

新しい「短冊型カード」を viz パターンに加える場合は、必ず `SPOT_SELECTOR` と CSS の `.has-spotlight > .X:not(.spotlight)` 群、`.X.spotlight` 群、モバイル `@media (max-width: 720px)` 群、すべてに新クラス名を追加する。
...
[6] 構文チェック
    ...
    └ JS 中括弧・括弧バランス(`node --check` で extracted script を確認推奨)
    └ 4ブレイクポイントで崩れない
    └ §9 の落とし穴をすべてチェック

[6.5] モバイル UX 動作確認(§8.5 が崩れていないか)
    └ 短冊カードタップで暗転 + 拡大 + ゴールドリング + 自動センタースクロール
    └ バックドロップタップ・Esc・再タップ で解除
    └ 3列 compare は `.three-col` クラスでマークされ、モバイルで横スワイプ + ドット連動
    └ 採点後スクロールは「正答」バナーが画面先頭に来る位置(`block:'center'` を使っていない)
    └ 水平スクロール SVG (`.viz-svg-wrap` `.family-tree-wrap`) でページ遷移しない
    └ サマリ到達でスコアがカウントアップ
    └ haptic が Android で動く、iOS でエラーが出ない
...
| `examples/01-01-information-and-media.html` | **最新完成リファレンス**(モバイル UI v2 完備。コピー元はこれ) |
| `examples/01-03-intellectual-property.html` | 同等機構を持つ別の完成品(モジュール構成や viz パターンの引き出しとして参照) |

新規単元を作るときは必ず `examples/01-01-information-and-media.html` をコピーしてから内容を差し替えること。ゼロから書こうとしない。例題ステージや練習問題ステージの中身は差し替えるが、`<style>` `<script>` 内の JS フック群(spotlight / haptic / animateCounter / isInsideHorizontalScroll / 採点後スクロール / カルーセルドット注入)は**絶対に削らない**。
