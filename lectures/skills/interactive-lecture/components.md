...
## APPENDIX リッチ図解(`.appx*`) ― 発展/参考スライド専用 ★ 2026-05 lec11 で確立

発展(参考)スライドを **`deep-card`(見出し+タグ+段落)の羅列で作らない**ための部品群。各回を「ヒーロー図 + 視覚的動線 + 構造化ポイント」で組む(`NEW-LECTURE-PLAYBOOK.md §5.5`)。canonical = lec11 参考 A〜F。CSS は lec11 `<style>` 内 `/* APPENDIX リッチ図解 */` ブロックに verbatim。

### クラス一覧
| クラス | 役割 |
|---|---|
| `.appx` | 発展スライドのラッパ(max-width 940px)。中で `sf-flow/pulse/float/grow` を **スライド本体スコープで再定義**(モーダル外でアニメを効かせるため。`@keyframes` はグローバルだがクラスは `.supp-card`/`#pointPop` 限定なので) |
| `.appx-hero` + `.appx-panel`(`.good`/`.bad`) + `.appx-vs` | **対比型ヒーロー**。2 パネルを左右、中央に丸い `vs`/`→`。`.good`=緑上線 `.bad`=赤上線で良し悪しを色で。各パネル = `.ap-h`(見出し)+ SVG + `.ap-note` |
| `.appx-figbox` + `.appx-cap` | **単図ヒーロー**(階段・処理フロー等)。SVG 1 枚 + 下に一言キャプション |
| `.appx-points` + `.appx-pt`(`.pt-tag` / `h4` / `p`) | **構造化ポイント**。図の下に 2 枚並べる。旧 `deep-card` の置き換え(タグ + 小見出し + 1 段落) |

### ヒーロー図の型(lec11 実例)
| 型 | 使う場面 | lec11 実例 |
|---|---|---|
| **対比(2 パネル)** | 良し悪し・A vs B | 参考A 2値(✓ノイズに強い)vs 多値(✗隣に化ける)。動くノイズ信号 `sf-flow` |
| **階段** | 段階的に増える量・単位 | 参考B bit→B→KB→MB→GB→TB(`sf-grow` staggered で段が伸びる/×8→×1024/例タグ `sf-float`) |
| **処理フロー** | 手順・変換(A→B→C) | 参考E 3=0011 →反転→ 1100 →+1→ 1101。変化ビットを `sf-pulse`、`marker-end` で矢印 |
| **リング/概念** | 一周・対応・循環 | 参考D 1111 →+1→ あふれて 0000(時計の比喩) |
| **重み分解** | 桁・ビットの重みを足す | 参考F 2 進小数 0.101 = 1/2+1/8 / 浮動小数点 S・E・M の 3 分割バー |

### 規律
- **動きは意味があるときだけ**(`§5` アニメ規律)。装飾の常時アニメは付けない。
- **論理順序**で並べる(手順より「なぜ」が先。例 負数=符号→概念→手順)。
- 1 トピック = 1 スライド。
- 図中の文字は **9px 以上**、`font-size="0"` の空 `text` を取り残さない。**SVG 内に `<br>` は不可**(改行は別 `<text>` で)。
