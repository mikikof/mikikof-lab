...
### 9.19 SVG 内に背景矩形を置くと「白い四角」が浮いて見える(2026-05 lec01 制作で発覚)

- **症状**: 各 law-illust SVG の冒頭に `<rect width="256" height="256" fill="url(#iBg)"/>` のような背景矩形を入れていた。コンテナ(`.def-box`, `.law-illust` 等)の中で **「白い四角」が浮いて自然なアイコン感が損なわれる**。ユーザーから「背景が四角く白くなっているのが非常に気になる、透過させて自然なアイコンにしたい」と指摘
- **原因**: SVG 自体に塗り背景を持たせると、コンテナとの境界が線として可視化される
- **対策**:
  - 全 SVG は **完全透過**(`<rect>` 背景禁止)
  - `.law-illust` の CSS background も `transparent` に。アクセントが必要なら `border-bottom: 1px dashed var(--navy-pale)` など細い線で代替
  - 背景の色味は **コンテナ側の CSS**(`.def-box` の `var(--navy-bg)` など)に任せる
- **canonical**: `articles/01-information-and-media/index.html` の law-illust SVG はすべて透過
- **過去事故**: lec01 制作時、14 個の Phosphor 化 SVG 全てに背景矩形を入れていた → 全削除して透過化

### 9.20 SVG 内 `<text>` で役割ラベルを書くと円のリングと重なって読めない(2026-05 lec01 制作で発覚)

- **症状**: 「② POINT」「残存性」「⑤ 生命情報」などのラベルを `<text x="..." y="..." font-family="...">残存性</text>` で SVG 内に書いていた。円・楕円のリング線、Phosphor アイコンの path、隣接要素と重なって**文字が読めない**。ユーザーから「上の添字がアイコンと被ってる、文字の可読性を上げたい」と指摘
- **原因**:
  - SVG 内テキストはブラウザのフォントレンダリング最適化が効きにくい(特に日本語)
  - 円の y 座標と text の y 座標を緻密に計算する必要があり、レスポンシブで崩れやすい
  - 微妙な位置調整を続けても根本解決にならない
- **対策(新ルール)**:
  - **SVG はアイコン本体だけ**を担う(`<path>` `<circle>` 等の図形要素のみ)
  - **役割ラベル・タグ・名称・サブ説明はすべて HTML 要素**として配置
    - `.icon-card-tag` (例: 「② POINT」) — `JetBrains Mono`、赤
    - `.icon-card-svg` (Phosphor アイコン本体)
    - `.icon-card-name` (例: 「残存性」) — 教科書体、navy
    - `.icon-card-sub` (例: 「空間を越える」) — Noto Sans、薄グレー
  - インタラクションは外側の `.icon-card` の `onclick` で取れる
- **canonical**: `articles/01-information-and-media/index.html` SLIDE 5(prop-card)/ SLIDE 12(med-card)。詳細は §14
- **例外**: 楕円の入れ子構造でレイヤを示すラベルなど、図と一体化した極小ラベルは SVG 内に残してよい(SLIDE 8 nest-svg)。ただし font-size 13+、リング線から 15px 以上離す
- **過去事故**: lec01 制作時、prop-svg 内の「② POINT」「③ POINT」「④ POINT」が円のリング上端に被って読めず → 全 HTML icon-card 形式に再構築
...
## 14. icon-card パターン(2026-05 lec01 で確立 / §9.20 + §13 統合)

「SVG 内テキストでラベルを書くと読めなくなる」(§9.20) と「OSS アイコンを単元統一で使う」(§13) を両立する標準パターン。**3 ノード以上の interactive 図解(N 者関係 / N 分類)で第一選択**。

### 14.1 構造

```html
<div class="icon-cards">
  <div class="icon-card prop-card" data-key="...">
    <div class="icon-card-tag">② POINT</div>           <!-- HTML テキスト -->
    <svg class="icon-card-svg" viewBox="0 0 256 256">  <!-- アイコン本体のみ -->
      [Phosphor の path data + design tokens]
    </svg>
    <div class="icon-card-name">残存性</div>            <!-- HTML テキスト -->
    <div class="icon-card-sub">空間を越える</div>       <!-- HTML テキスト(任意) -->
  </div>
  ...
</div>
<div class="cards-hint">タップして詳細を表示</div>
```

### 14.2 標準 CSS

```css
.icon-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }
.icon-card {
  background: white;
...
.icon-card:hover {
  ...
  box-shadow: 0 6px 18px rgba(15,40,71,0.1);
}
.icon-card.selected {
  border-color: var(--accent-red);
  background: linear-gradient(180deg, rgba(201,169,97,0.10) 0%, white 100%);
...
.icon-card-svg { width: 78px; height: 78px; display: block; margin: 4px 0 2px; }
.icon-card-name {
...
### 14.3 SVG 内の Phosphor → design tokens 変換テンプレート

```svg
<svg class="icon-card-svg" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="iN{n}" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="#5d8eb3"/>
      <stop offset="100%" stop-color="#0F2847"/>
    </linearGradient>
    <linearGradient id="iG{n}" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="#f4d995"/>
      <stop offset="100%" stop-color="#8a6d1f"/>
    </linearGradient>
    <filter id="iS{n}" x="-20%" y="-20%" width="140%" height="140%">
  ...
  <!-- Phosphor の opacity 0.2 path → fill="url(#iG{n})" opacity="0.5" -->
  <!-- Phosphor の stroke="currentColor" path → stroke="url(#iN{n})" + filter="url(#iS{n})" を主要要素に -->
</svg>
```

**重要なルール**:
- `viewBox="0 0 256 256"` は **そのまま使う**(Phosphor の標準。scaling 不要)
- `<rect width="256" height="256" fill="..."/>` の **背景矩形は入れない**(§9.19)
- defs の id は SVG ごとにユニーク化(`iN1`, `iN2`, ...)。同名 ID を複数 SVG で再利用すると最初のものが使われ、後続が壊れる
- 主役 Phosphor stroke = `url(#iN{n})` + drop-shadow filter / 副 fill = `url(#iG{n})` opacity 0.5 / 警告系は red を使う(SLIDE 18 ⑫炎上)

### 14.4 JavaScript の selectXxx 実装

```js
function selectProp(key) {
  document.querySelectorAll('.prop-card').forEach(c => c.classList.remove('selected'));
  const node = document.querySelector('.prop-card[data-key="' + key + '"]');
  if (node) node.classList.add('selected');
  const panel = document.getElementById('propPanel');
  const d = propData[key];
  if (panel && d) panel.innerHTML = '...';
}
```

`resetAllInteractions` 内でも `.icon-card.selected` を扱うか、`.prop-card.selected, .med-card.selected` のように個別に扱う。

### 14.5 適用判断

**第一選択: icon-card**
- 3〜6 個の並列ノード(N 者関係、N 分類、N 役割など)
- 各ノードに「タグ + アイコン + 名称 + サブ説明」を出したい場合

**第二選択: SVG 一体型(従来の interactive-svg-node)**
- 包含構造(楕円の入れ子)など、ノード間の **空間関係に意味** がある場合
- ただしラベル位置はリングから十分離す

**禁止: SVG 内 `<text>` で「② POINT」のような役割ラベル**(§9.20)
